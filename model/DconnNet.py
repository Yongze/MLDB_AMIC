...
class DconnNet(nn.Module):
    def __init__(self, num_class=1):
        ...
        self.fb5 = FeatureBlock(
            512 * self.exp, 256 * self.exp, relu=False, last=True
        )  # 256
    ...
    def forward(self, x):
        ...
        c2 = self.backbone.layer1(x)  # 1/4   34:64; 101:256
        c3 = self.backbone.layer2(c2)  # 1/8   34:128; 101:512
        c4 = self.backbone.layer3(c3)  # 1/16   34:256; 101:1024
        c5 = self.backbone.layer4(c4)  # 1/32   34:512; 101:2048
...
class SDE_module(nn.Module):
    def __init__(self, in_channels, out_channels, num_class):
        ...
        self.final_conv = nn.Sequential(
            nn.Dropout2d(0.1, False), nn.Conv2d(in_channels, out_channels, 1)
        )
    ...
    def forward(self, x, d_prior):
        ...
        feat1 = self.att1(
            x[:, : self.inter_channels], enc_feat[:, 0 : self.inter_channels]
        )
        feat2 = self.att2(
        ...
        feat = torch.cat(
            [feat1, feat2, feat3, feat4, feat5, feat6, feat7, feat8], dim=1
        )
...
class DANetHead(nn.Module):
    def __init__(self, in_channels, inter_channels, norm_layer=nn.BatchNorm2d):
        ...
        self.conv8 = nn.Sequential(
            nn.Dropout2d(0.1, False), nn.Conv2d(inter_channels, inter_channels, 1)
        )
