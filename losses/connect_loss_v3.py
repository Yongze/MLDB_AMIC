...
def Bilateral_voting(c_map, hori_translation, verti_translation):
    ...
    right_above = (torch.bmm(right_above.view(-1, row, column), hori_translation.view(-1, column, column))).view(
...
class connect_loss(nn.Module):
    ...
    def multi_class_forward(self, c_maps, onehotmask):
        #######
        ...
        # onehotmask = F.one_hot(target.long(), self.num_class)
        ...
        # onehotmask = target.float()

        ### get your connectivity_mask ###
        # (B, 8*C, H, W)
        con_target = connectivity_matrix(onehotmask, self.num_class)
        ...
        dice_l = 0
        pred = F.sigmoid(c_maps[1])
        for j in range(self.num_class):
            dice_l += self.dice_loss(pred[:, j, :, :], onehotmask[:, j], j - 1)

        ce_loss = self.ce_loss(c_maps[1], onehotmask)
        conn_l = self.BCEloss(F.sigmoid(c_maps[0]), con_target)
        bicon_l = self.BCEloss(bicon_map, con_target)
    ...
    #     ### get edges gt###
    #     class_conn = con_target.view(
    #         [c_map.shape[0], self.num_class, 8, c_map.shape[2], c_map.shape[3]]
    #     )
    #     sum_conn = torch.sum(class_conn, dim=2)
    ...
    #     else:
    #         bicon_l = self.BCEloss(bicon_map.squeeze(1), con_target)
    #         # + bce_loss# +loss_out_dice# +sum_l # + edge_l+loss_out_dice
