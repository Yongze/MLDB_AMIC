...
def get_lr(base_lr, mode, epoch, epoch_num, gamma=None, step=None, db_size=None):
    ...
    if mode == "warm-up-step":
        ...
        lr, momentum = get_triangle_lr(
            base_lr, max_lr, epoch_num * db_size, niter, ratio=1.0
        )
        return lr, momentum
...
def get_triangle_lr(
    ...
    if cur < first:
        ...
        lr = ((base_lr - min_lr) * cur + min_lr * first - base_lr * total_steps) / (
            first - total_steps
        )
    if isinstance(momentums, int):
        ...
        if cur < first:
            momentum = momentums[0] + (momentums[1] - momentums[0]) * np.maximum(
                0.0, 1.0 - x
            )
        else:
