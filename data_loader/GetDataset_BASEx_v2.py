...
class MyDataset_BASEx_v2(data.Dataset):  #
    def __init__(self, data_root, resize, train=True):
        self.img_ls = []
        self.imgs = []
        self.masks = []
        ...
        for root in data_root:
            tmp = GetDataset(root)
            leng = len(tmp)
            train_num = round(leng * self.train_ratio)
            if self.train_mode:
                self.img_ls.extend(tmp[:train_num])
            else:
                self.img_ls.extend(tmp[train_num:])
